...
- **YouTube PO Token (optional):** Marked IPs (emulator, datacenter, VPN) get YouTube's bot check on every InnerTube client, and without a token the only usable format is `itag=18` (360p muxed) — the real cause behind "YouTube sounds bad". Fix: run the local bgutil provider (`scripts/pruebas/pot_local.sh`, Node only, no Docker) and on the emulator map it with `adb reverse tcp:4416 tcp:4416`. Full guide: `go_backend/POT_TOKEN_YOUTUBE.md`.
