...
function cleanup() {
  try {
    storage.set("am_state", JSON.stringify({
      token: state.token,
...
function fetchAlbum(albumID) {
  ...
  var data = apiGet("albums/" + albumID + "?include=tracks,artists&extend=editorialArtwork");
...
function fetchPlaylist(playlistID) {
  ...
  var data = apiGet("playlists/" + playlistID + "?include=tracks&extend=editorialArtwork");
...
function customSearch(searchQuery, options) {
  ...
  if (searchResults.albums && (!isFiltered || filter === "albums")) {
    ...
    for (var j = 0; j < albumsData.length; j++) {
      ...
      // Apple Music's albums endpoint includes singles and EPs mixed in with
      // full albums. When specifically searching for albums, filter them out.
      if (isFiltered && filter === "albums" && albAttr.isSingle) {
        continue;
      }
      results.push({
        id: albumsData[j].id || "",
