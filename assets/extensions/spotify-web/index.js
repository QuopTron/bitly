...
function restGet(url, allowRetry) {
  ...
  const response = http.get(url, headers);
  if (!response || response.error) {
    throw new Error("REST request failed: " + (response ? response.error : "no response"));
  }
  if (response.statusCode === 401 && allowRetry !== false) {
    resetAuthState();
    ensureInitialized();
...
function fetchHomeFeed() {
  ...
  try {
    const data = restGet("https://api.spotify.com/v1/browse/new-releases?country=US&limit=20");
    const albums = data && data.albums && data.albums.items ? data.albums.items : [];
    const items = [];
    for (let i = 0; i < albums.length && items.length < 12; i++) {
      const album = albums[i] || {};
      const name = album.name || "";
      if (!name) continue;
      const artistNames = (album.artists || []).map(function (a) {
        return a.name || "";
    ...
    if (items.length > 0) {
      sections.push({ uri: "sp:new-releases", title: "New Releases", items: items });
    }
  } catch (e) {
  ...
  try {
    const data = restGet("https://api.spotify.com/v1/browse/featured-playlists?country=US&limit=20");
    const playlists = data && data.playlists && data.playlists.items ? data.playlists.items : [];
    const items = [];
    for (let i = 0; i < playlists.length && items.length < 12; i++) {
      ...
      items.push({
        ...
        total_tracks: playlist.tracks && playlist.tracks.total ? playlist.tracks.total : 0,
        provider_id: "spotify-web"
      });
    }
    if (items.length > 0) {
      sections.push({ uri: "sp:featured-playlists", title: "Featured Playlists", items: items });
    }
  } catch (e) {
  ...
  if (sections.length > 0) {
    log.info("Fetched", sections.length, "sections from Spotify home feed (REST)");
  ...
  try {
    ...
    if (parsed && parsed.success && parsed.sections && parsed.sections.length > 0) {
      return parsed;
    }
  } catch (e) {
