...
function artistNamesMatch(expected, found) {
  ...
  for (var i = 0; i < aParts.length; i++) {
    for (var j = 0; j < bParts.length; j++) {
      ...
      if (
        aParts[i].indexOf(bParts[j]) >= 0 ||
        bParts[j].indexOf(aParts[i]) >= 0
      ) {
        return true;
      }
...
function qobuzTrackMatchesRequest(
  track,
  isrc,
  trackName,
  artistName,
...
function scoreTrackCandidate(query, track) {
  ...
  if (
    titlesMatch(query, track.title) ||
    titlesMatch(query, trackDisplayTitle(track))
  ) {
    score += 900;
  }
...
function selectBestSearchTrack(
  ...
  if (matches.length) {
    matches.sort(function (a, b) {
      if (
        Number(a.maximum_bit_depth || 0) !== Number(b.maximum_bit_depth || 0)
      ) {
        return (
          Number(b.maximum_bit_depth || 0) - Number(a.maximum_bit_depth || 0)
        );
      }
...
function albumTypeFromRelease(releaseType, productType, totalTracks) {
  var kind = String(releaseType || productType || "")
    .trim()
    .toLowerCase();
  switch (kind) {
    case "album":
...
function qobuzAudioQualityLabel(track) {
  ...
  var rateDisplay =
    sampleRate % 1 === 0 ? String(sampleRate) : sampleRate.toFixed(1);
...
function selectTracksFromAlbumSearch(query, summaries, limit) {
  ...
  candidates.sort(function (a, b) {
    ...
    if (
      Number(a.track.maximum_bit_depth || 0) !==
      Number(b.track.maximum_bit_depth || 0)
    ) {
      return (
        Number(b.track.maximum_bit_depth || 0) -
        Number(a.track.maximum_bit_depth || 0)
...
function searchOne(query, filter, limit) {
  ...
  var normalizedFilter = String(filter || "")
    .trim()
    .toLowerCase();
...
function download(trackID, quality, outputPath, onProgress) {
  try {
    ...
    for (var attempt = 0; attempt < maxDownloadAttempts; attempt++) {
      try {
        downloadInfo = resolveDownloadInfo(
          trackID,
          quality,
        ...
        if (previewMessages.length) {
          return {
            success: false,
            error_message:
              "All Qobuz candidates returned preview-length audio: " +
      ...
      var downloadResult = downloadDirectFile(
      ...
      if (!downloadResult || !downloadResult.success) {
        ...
        return {
          success: false,
          error_message:
            "Failed to download Qobuz stream: " +
      ...
      previewMessages.push(
        String(downloadInfo.provider || "provider") +
          "@" +
          String(downloadInfo.qualityCode || "") +
          ": " +
          validation.message,
      );
      rejectedDownloadCandidates[String(downloadInfo.candidateKey || "")] =
        true;
      rejectedDownloadCandidates[
        "url:" + String(downloadInfo.directURL || "").trim()
      ] = true;
      log.warn(
