...
function albumImage(album) {
  ...
  if (Array.isArray(album.images) && album.images.length > 0) {
    ...
    if (first && typeof first === "object") {
      var fromArr = firstNonEmpty(first.url, first.large, first.small, first.thumbnail);
...
function getHomeFeed() {
  ...
  try {
    ...
    for (var i = 0; i < albums.length && albumItems.length < 12; i++) {
      ...
      albumItems.push({
        name: name,
        artists: joinArtistNames(item.artists || [], item.artist && item.artist.name),
        type: "album",
    ...
    if (albumItems.length > 0) {
      sections.push({ uri: "qbz:featured", title: "Novedades de Qobuz", items: albumItems });
    }
  } catch (e) {
