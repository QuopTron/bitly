var CONFIG = {
  ...
  pageSize: 50,
  downloadProviders: [
    { name: "zarz", url: "https://api.zarz.moe/v1/dl/qbz" }
  ]
};
...
function getMetadataJSON(path, params) {
  ...
  try {
    ...
    log.warn("[QobuzWeb] Primary metadata failed for " + path + ": " + primaryError.message);
...
function searchAlbumsViaAPI(query, limit) {
  log.info("[QobuzWeb] Searching albums: query=\"" + String(query || "").trim() + "\" limit=" + limit);
  var payload = getMetadataJSON("album/search", {
    query: String(query || "").trim(),
    limit: limit
  });
  var items = payload && payload.albums && payload.albums.items ? payload.albums.items : [];
  log.info("[QobuzWeb] album/search returned " + items.length + " items");
  return items;
}
