...
func ScanLibraryFolder(folderPath string) (string, error) {
	...
	// Load existing database state for incremental scan
	existingFiles, _ := GetExistingModTimes()
	GoLog("[LibraryScan] Loaded %d existing files from database for incremental comparison\n", len(existingFiles))
	...
	for i, fileInfo := range audioFileInfos {
		...
		if dbModTime, exists := existingFiles[filePath]; exists && dbModTime == fileInfo.modTime {
			skippedCount++
			continue
		}
		...
		if ext == ".cue" {
			...
			_ = UpsertLibraryBatch(cueResults)
			continue
		}
		...
		// Batch save to database every 50 items
		if len(results)%50 == 0 {
			_ = UpsertLibraryBatch(results[len(results)-50:])
		}
	}

	// Final batch save for items not yet saved
	remaining := len(results) % 50
	if remaining > 0 {
		_ = UpsertLibraryBatch(results[len(results)-remaining:])
	}

	// 4. DETECT DELETIONS: Files in DB but not on disk
	currentPathSet := make(map[string]bool, len(audioFileInfos))
	for _, f := range audioFileInfos {
		currentPathSet[f.path] = true
	}

	var deletedPaths []string
	for dbPath := range existingFiles {
		// Handle CUE tracks specially (strip #track suffix)
		checkPath := dbPath
		if idx := strings.LastIndex(dbPath, "#track"); idx > 0 {
			checkPath = dbPath[:idx]
		}

		if !currentPathSet[checkPath] {
			deletedPaths = append(deletedPaths, dbPath)
		}
	}

	if len(deletedPaths) > 0 {
		GoLog("[LibraryScan] Cleaning up %d deleted paths from database\n", len(deletedPaths))
		_ = DeleteLibraryPaths(deletedPaths)
	}
	...
	GoLog("[LibraryScan] Scan complete: %d tracks found, %d errors. All saved to database.\n", len(results), errorCount)
