...
func (m *extensionManager) LoadExtensionFromFile(filePath string) (*loadedExtension, error) {
	GoLog("[Extension] LoadExtensionFromFile called with path: %s", filePath)
	if strings.TrimSpace(filePath) == "" {
		GoLog("[Extension] LoadExtensionFromFile: empty file path")
		return nil, fmt.Errorf("invalid file path")
	}
	if fi, err := os.Stat(filePath); err != nil {
		GoLog("[Extension] LoadExtensionFromFile: stat error for %s: %v", filePath, err)
	} else {
		GoLog("[Extension] LoadExtensionFromFile: found file %s (size=%d)", filePath, fi.Size())
	}

	// Allow files that may not have the expected suffix as long as they are valid zip packages
	suffixOk := strings.HasSuffix(strings.ToLower(filePath), ".bitly-ext")
	zipReader, err := zip.OpenReader(filePath)
	if err != nil {
		if !suffixOk {
			GoLog("[Extension] LoadExtensionFromFile: file not a zip and suffix missing: %s", filePath)
	...
	if !suffixOk {
		GoLog("[Extension] LoadExtensionFromFile: file does not have .bitly-ext suffix but is a valid zip, proceeding: %s", filePath)
	}
...
func (m *extensionManager) LoadExtensionsFromDirectory(dirPath string) ([]string, []error) {
	...
	for _, entry := range entries {
		...
		} else if strings.HasSuffix(strings.ToLower(entry.Name()), ".bitly-ext") {
...
func (m *extensionManager) UpgradeExtension(filePath string) (*loadedExtension, error) {
	// Accept packages even if the filename doesn't strictly end with .bitly-ext as long as the file is a valid zip
	suffixOk := strings.HasSuffix(strings.ToLower(filePath), ".bitly-ext")
	zipReader, err := zip.OpenReader(filePath)
	if err != nil {
		if !suffixOk {
	...
	defer zipReader.Close()
	if !suffixOk {
		GoLog("[Extension] UpgradeExtension: file does not have .bitly-ext suffix but is a valid zip, proceeding: %s", filePath)
	}
...
func (m *extensionManager) checkExtensionUpgradeInternal(filePath string) (*ExtensionUpgradeInfo, error) {
	...
	defer zipReader.Close()
	if !suffixOk {
		GoLog("[Extension] checkExtensionUpgradeInternal: file does not have .bitly-ext suffix but is a valid zip, proceeding: %s", filePath)
	}
