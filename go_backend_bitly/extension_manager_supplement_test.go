...
`
	pkgV1 := filepath.Join(dir, "manager-ext-v1.Bitly-ext")
	createTestExtensionPackage(t, pkgV1, "manager-ext", "1.0.0", js, map[string]string{"../unsafe.txt": "skip"})
	pkgV2 := filepath.Join(dir, "manager-ext-v2.Bitly-ext")
	...
	if _, err := manager.LoadExtensionFromFile(filepath.Join(dir, "missing.Bitly-ext")); err == nil {
