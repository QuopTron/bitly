...
// QQMusicClient fetches lyrics from QQ Music via the Paxsenix proxy.
...
// NewQQMusicClient creates a new QQ Music lyrics client.
...
func (c *QQMusicClient) fetchLyricsByMetadata(trackName, artistName string, durationSec float64) (string, error) {
	...
	req, err := http.NewRequest("POST", "https://lyrics.paxsenix.org/qq/lyrics-metadata", strings.NewReader(string(payloadBytes)))
	if err != nil {
...
func formatQQLyricsMetadataToLRC(rawJSON string, multiPersonWordByWord bool) (string, error) {
	...
	return formatPaxContent("Syllable", response.Lyrics, multiPersonWordByWord, false), nil
}

// FetchLyrics fetches QQ Music lyrics for a track.
func (c *QQMusicClient) FetchLyrics(trackName, artistName string, durationSec float64, multiPersonWordByWord bool) (*LyricsResponse, error) {
	...
	if err != nil {
		if fallback, fbErr := formatPaxLyricsToLRC(rawLyrics, multiPersonWordByWord, false); fbErr == nil {
			lrcText = fallback
		} else {
			lrcText = rawLyrics
		}
	...
	if len(lines) > 0 {
		return &LyricsResponse{Lines: lines, SyncType: "LINE_SYNCED", Provider: "QQ Music", Source: "QQ Music"}, nil
	}

	resultLines := PlainTextLyricsLines(lrcText)
	if len(resultLines) > 0 {
		return &LyricsResponse{Lines: resultLines, SyncType: "UNSYNCED", Provider: "QQ Music", Source: "QQ Music"}, nil
	}
