...
// GetLovedTracksV2 returns all loved tracks as JSON.
func GetLovedTracksV2() (string, error) {
	db, err := Get()
	if err != nil {
		return "", err
	}
	...
	rows, err := db.Query(`
		SELECT lt.track_id, lt.added_at,
...
// RemoveLovedTrackV2 removes a track from loved_tracks.
func RemoveLovedTrackV2(trackID, isrc, trackName, artistName string) error {
	db, err := Get()
	if err != nil {
		return err
	}
	canonID := CanonicalTrackID(isrc, trackName, artistName)
	db.Exec("DELETE FROM loved_tracks WHERE track_id = ?", canonID)
	if trackID != "" && trackID != canonID {
		db.Exec("DELETE FROM loved_tracks WHERE track_id = ?", trackID)
	}
	return nil
}
