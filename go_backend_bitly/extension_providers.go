...
func DownloadWithExtensionFallback(req DownloadRequest) (*DownloadResponse, error) {
	...
	// Try Tidal Monochrome to enrich missing ISRC before falling back to metadata search
	if req.ISRC == "" && req.TrackName != "" && req.ArtistName != "" {
		newISRC := TidalMonochromeEnrichISRC(&req)
		if newISRC != "" {
			GoLog("[DownloadWithExtensionFallback] ISRC enriched via Tidal: %s\n", newISRC)
		}
	}

	needsMetadataSearch := req.TrackName != "" && req.ArtistName != "" &&
		(req.ISRC == "" || req.AlbumName == "" || req.ReleaseDate == "")
	if needsMetadataSearch {
		searchQuery := req.TrackName + " " + req.ArtistName
		GoLog("[DownloadWithExtensionFallback] Metadata incomplete (source=%s), searching providers for: %s\n", req.Source, searchQuery)
	...
	for _, providerID := range priority {
		...
		if providerID == "qobuz_kennyy" {
			result, dlErr := downloadViaQobuzKennyy(&req)
			if dlErr != nil {
				GoLog("[DownloadWithExtensionFallback] qobuz_kennyy failed: %v\n", dlErr)
				lastErr = dlErr
				continue
			}
			return result, nil
...
func buildOutputPath(req DownloadRequest) string {
	...
	if strings.TrimSpace(outputDir) == "" {
		outputDir = filepath.Join(os.TempDir(), "Bitly-downloads")
	}
