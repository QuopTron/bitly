...
// PickSafTree is a stub for gomobile binding compatibility.
// SAF tree picking should be handled natively from Kotlin/Android.
func PickSafTree() (string, error) {
	result := map[string]interface{}{
		"success": false,
		"message": "SAF tree picking must be handled natively from Android/Kotlin",
		"treeUri": "",
	}
	jsonBytes, _ := json.Marshal(result)
	return string(jsonBytes), fmt.Errorf("SAF picker not available in Go backend - use native Android picker")
}
...
func FetchMusicBrainzAlbumArtistByISRC(isrc string, albumName string) (string, error) {
	return GetMusicBrainzClient().FetchAlbumArtistByISRC(isrc, albumName)
}

func FetchMusicBrainzGenreByISRC(isrc string) (string, error) {
	return GetMusicBrainzClient().FetchGenreByISRC(isrc)
}
...
func GetTranslatedLyricsLRC(spotifyID, trackName, artistName string, durationMs int64, language string) (string, error) {
	if strings.TrimSpace(language) == "" {
		return "", fmt.Errorf("translation language is required")
	}
	client := NewMusixmatchClient()
	durationSec := float64(durationMs) / 1000.0
	lyrics, err := client.FetchLyricsInLanguage(trackName, artistName, durationSec, language)
	if err != nil {
		GoLog("[GetTranslatedLyricsLRC] Musixmatch translation failed: %v\n", err)
		return "", err
	}
...
// SetTranslationLanguageJSON saves the preferred translation language to DB
func SetTranslationLanguageJSON(language string) error {
	return SaveTranslationLanguage(language)
}

// GetTranslationLanguageJSON loads the preferred translation language from DB
func GetTranslationLanguageJSON() (string, error) {
	lang, err := LoadTranslationLanguage()
	if err != nil {
		return "", err
	}
	if lang == "" {
		return "es", nil // default to Spanish
	}
	return lang, nil
}
...
func GetProviderMetadataJSON(providerID, resourceType, resourceID string) (string, error) {
	...
	case "qobuz_kennyy":
		return GetQobuzKennyyMetadata(resourceType, resourceID)
	case "tidal_monochrome":
		return GetTidalMonochromeMetadata(resourceType, resourceID)
	default:
...
func InitExtensionSystem(extensionsDir, dataDir string) error {
	// Enable verbose Go logging for extension system to assist debugging on mobile
	SetLoggingEnabled(true)

	manager := getExtensionManager()
	if err := manager.SetDirectories(extensionsDir, dataDir); err != nil {
		LogError("Extension", "failed to set directories: %v", err)
		return err
	}

	settingsStore := GetExtensionSettingsStore()
	if err := settingsStore.SetDataDir(dataDir); err != nil {
		LogError("Extension", "failed to set settings store data dir: %v", err)
		return err
	}

	LogInfo("Extension", "InitExtensionSystem: extensionsDir=%s dataDir=%s", extensionsDir, dataDir)
	return nil
}
...
// SetDownloadFallbackExtensionIdsJSON is an alias for gomobile binding compatibility
func SetDownloadFallbackExtensionIdsJSON(extensionIDsJSON string) error {
	return SetExtensionFallbackProviderIDsJSON(extensionIDsJSON)
}
...
func BootstrapEssentialExtensions() (string, error) {
	...
	_, err := store.fetchRegistry(false)
	if err != nil {
		GoLog("[Extension] Bootstrap: failed to fetch registry: %v", err)
		return "", err
	}

	installedCount := 0

	// Create a normalization map for currently installed extensions
	manager.mu.RLock()
	normalizedInstalled := make(map[string]bool)
	for id := range manager.extensions {
		normalizedInstalled[normalizeStoreID(id)] = true
	}
	manager.mu.RUnlock()

	for _, id := range essentialIDs {
		normID := normalizeStoreID(id)

		manager.mu.RLock()
		_, existsExact := manager.extensions[id]
		existsNorm := normalizedInstalled[normID]
		manager.mu.RUnlock()

		if existsExact || existsNorm {
			continue
		}

		GoLog("[Extension] Bootstrap: Auto-installing essential extension: %s", id)

		// Create a temporary directory for the download
		tempDir := filepath.Join(manager.extensionsDir, ".bootstrap", id)
		os.MkdirAll(tempDir, 0755)
		destPath := filepath.Join(tempDir, id+".bitly-ext")

		err := store.downloadExtension(id, destPath)
		if err != nil {
			GoLog("[Extension] Bootstrap: Failed to download %s: %v", id, err)
			continue
		}

		_, err = manager.LoadExtensionFromFile(destPath)
		if err != nil {
			GoLog("[Extension] Bootstrap: Failed to load %s: %v", id, err)
			continue
		}

		// Enable essential extensions by default using the safe public method
		manager.SetExtensionEnabled(id, true)

		installedCount++
		GoLog("[Extension] Bootstrap: Successfully installed %s", id)
	...
	return fmt.Sprintf("Installed %d extensions", installedCount), nil
}
...
func buildStoreExtensionDestPath(destDir, extensionID string) (string, error) {
	...
	return filepath.Join(destDir, safeExtensionID+".bitly-ext"), nil
}
...
func InitMasterDatabaseJSON(path string) error {
	return InitMasterDatabase(path)
}

func SearchLibraryMasterJSON(query string, limit int) (string, error) {
	results, err := SearchLibrary(query, limit)
	if err != nil {
		return "[]", err
	}
	jsonBytes, err := json.Marshal(results)
...
// YouTube functions (exported for Android bridge)
func SearchYouTubeVideoJSON(trackName, artistName string) (string, error) {
	url, err := SearchYouTubeVideo(trackName, artistName)
	if err != nil {
		return "", err
	}
	return url, nil
...
func DownloadYouTubeVideoJSON(trackName, artistName, outputPath string) (string, error) {
	path, err := DownloadYouTubeVideo(trackName, artistName, outputPath)
	if err != nil {
		return "", err
	}
	return path, nil
...
// CheckGitHubUpdateJSON checks for updates on GitHub releases
// Parameters: JSON string with {"channel": "stable"|"preview", "current_version": "x.y.z", "repo": "owner/repo"}
// Returns: JSON string with update info or error
func CheckGitHubUpdateJSON(paramsJSON string) (string, error) {
	return checkGitHubUpdateInternal(paramsJSON), nil
}
