...
func TestRescueRaceNoDeadlockStuckWorker(t *testing.T) {
	...
	start := time.Now()
	url, name, _ := carreraRescue(reg, []string{"stuck", "slow", "fast"}, 2*time.Second, 2, func(n string, p provider.Provider) (string, bool) {
...
func TestRescueRaceAllStuck(t *testing.T) {
	...
	start := time.Now()
	url, name, _ := carreraRescue(reg, []string{"a", "b", "c"}, 500*time.Millisecond, 2, func(n string, p provider.Provider) (string, bool) {
		u, err := p.GetStreamURL(n, "high")
		if err != nil || u == "" {
			return "", false
		}
...
func TestRescueRaceFastWins(t *testing.T) {
	...
	url, name, _ := carreraRescue(reg, []string{"slow", "fast"}, time.Second, 2, func(n string, p provider.Provider) (string, bool) {
		u, err := p.GetStreamURL(n, "high")
		if err != nil || u == "" {
			return "", false
		}
