  ...
  Future<bool> _procesarItemFallido(
    ...
    if (stateKey == _idTrackActualCola) {
      ...
      if (outputPath.isNotEmpty) {
        try {
          ...
          if (await opFile.exists() && await _esAudioDecodificable(opFile)) {
            ...
            _log.i(
              '[poll] $rawId: Go reporta failed pero outputPath es reproducible: $outputPath — persistiendo',
            );
          }
        } catch (e) {
        ...
        if (playableAlt == null && outputPath.contains('.tmp.')) {
          try {
            ...
            if (await ntFile.exists() && await _esAudioDecodificable(ntFile)) {
              playableAlt = nonTmp;
              _log.i(
                '[poll] $rawId: Go reporta failed pero el path non-tmp es reproducible: $nonTmp — persistiendo',
              );
            }
          } catch (e) {
            debugPrint("[Descargas] $e");
      ...
      if (playableAlt != null) {
        ...
        _log.i(
          '[poll] $rawId: Go reporta failed pero se encontró alternativa: $playableAlt — persistiendo como completado',
        );
        try {
      ...
      if (count >= _maxPollFallidosSinArchivo) {
        ...
        _log.i(
          '[poll] $rawId: Go reporta failed pero la cola está activa — sin archivo aún ($count/$_maxPollFallidosSinArchivo), manteniendo enProgreso (outputPath=$outputPath)',
        );
        dl[stateKey] = const DatosEstadoDescarga(
