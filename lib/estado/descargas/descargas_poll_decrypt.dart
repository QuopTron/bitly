...
mixin DescargasPollDecrypt on DescargasPollCompletado {
  ...
  Future<({String? rutaReproducible, bool continuar})> _desencriptarSiNecesario(
    ...
    if (_decryptClienteHecho.contains(rawId) ||
  ...
  Future<({String? rutaReproducible, bool continuar})>
  _desencriptarConReintentos(
    String rawId,
    String stateKey,
    String decKey,
    String outExt,
    ...
    try {
      ...
      if (await probeFile.exists() && await _esAudioDecodificable(probeFile)) {
        _log.i(
          '[poll] $rawId: flag encriptado pero el archivo ya es reproducible: $playablePath — saltando decrypt',
        );
    ...
    if (altPath != null) {
      _log.i(
        '[poll] $rawId: archivo encriptado no reproducible pero se encontró alternativa: $altPath — saltando decrypt',
      );
      _decryptClienteHecho.add(rawId);
      _fallosDecryptCount.remove(rawId);
      try {
    ...
    final decrypted = await _desencriptarArchivoDescargado(
      playablePath,
      decKey,
      outExt,
      inputFormat,
    ...
    if (decrypted == null || decrypted.isEmpty) {
      ...
      if (attempts < _maxReintentosDecrypt) {
        _log.w(
          '[poll] decrypt falló para $rawId (intento $attempts/$_maxReintentosDecrypt), se reintentará el próximo poll',
        );
      ...
      if (altFinal != null) {
        _log.i(
          '[poll] decrypt falló para $rawId pero existe alternativa: $altFinal — manteniendo completado',
        );
      ...
      if (stateKey == _idTrackActualCola) {
        _log.i(
          '[poll] $rawId: decrypt falló pero la cola está activa — manteniendo enProgreso',
        );
        dl[stateKey] = const DatosEstadoDescarga(
          estado: EstadoDescarga.enProgreso,
          progreso: 0.95,
