...
mixin DescargasReintentar on DescargasColaVerificar {
  /// Borra TODO el estado de poll de un track para que un reintento arranque
  /// de cero. Sin esto, [_completadosPersistidos] haría que el poll saltee la
  /// entrada nueva del tracker (mismo rawId) y el reintento no descargaría
  /// nada: parecería que "reintentar" no hace nada.
  void _limpiarEstadoPollTrack(String normalizedId, String audioId) {
    final rawId = '${normalizedId}_audio';
    _decryptClienteSaltado.remove(rawId);
    _decryptClienteHecho.remove(rawId);
    _fallosDecryptCount.remove(rawId);
    _completadosPersistidos.remove(rawId);
    _carreraResuelta.remove(rawId);
  ...
  void reintentarTracksFallidosLote(String batchKey) {
    ...
    for (final t in fallidos) {
      ...
      dl[audioId] = const DatosEstadoDescarga(
        estado: EstadoDescarga.enCola,
        progreso: 0.0,
      );
      // Limpiar TODO el estado de poll para que el reintento procese desde
      // cero (misma limpieza que usa el reintento en sitio de la cola).
      _limpiarEstadoPollTrack(normalizedTid, audioId);
