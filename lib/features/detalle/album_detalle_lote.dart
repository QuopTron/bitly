// ─────────────────────────────────────────────────────────────
// album_detalle_lote.dart — PART de album_detalle_pagina.dart:
// reconstruye el DetalleAlbum desde el lote descargado cuando no
...
// Parte del flujo: Detalle → álbum (fallback offline).
// ─────────────────────────────────────────────────────────────

part of 'album_detalle_pagina.dart';

/// Arma un DetalleAlbum desde los lotes descargados cuando no hay red.
Future<DetalleAlbum?> _construirDesdeLote(_AlbumDetallePaginaState st) async {
  ...
  if (lote == null && st.widget.source.isNotEmpty) {
    lote = await dlCache.getLotePorItem('album', st.widget.albumId, '');
  }
  ...
  return DetalleAlbum(
    id: lote.itemId ?? st.widget.albumId,
    name: lote.name ?? '',
