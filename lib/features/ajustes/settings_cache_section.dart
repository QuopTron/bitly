...
class _SettingsCacheSectionState extends State<SettingsCacheSection> {
  ...
  Widget _sizeSelector(Responsive r, Color onBg, Color glowColor) {
    ...
    // Si el valor guardado ya no está disponible (p.ej. bajó el límite del
    // plan), usamos la opción válida más cercana para no romper el dropdown
    // ("There should be exactly one item with value X" -> pantalla roja).
    final valorSeguro = options.contains(currentMb)
        ? currentMb
        : options.isEmpty
            ? 200
            : options.reduce((a, b) =>
                (a - currentMb).abs() <= (b - currentMb).abs() ? a : b);
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      DropdownButtonFormField<int>(
        ...
        items: options.map((mb) {
          ...
          final isCurrent = mb == valorSeguro;
