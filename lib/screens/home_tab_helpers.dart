...
_RecentAccessView _buildRecentAccessViewData(
  ...
  for (final h in historyItems) {
    ...
    if (existing == null) {
      ...
      existing.count++;
      if (existing.mostRecent.downloadedAt == null ||
          (h.downloadedAt != null &&
              h.downloadedAt!.isAfter(existing.mostRecent.downloadedAt!))) {
  ...
  for (final aggregate in albumGroups.values) {
    ...
    final recent = RecentAccessItem(
      ...
      accessedAt: mostRecent.downloadedAt ?? DateTime.fromMillisecondsSinceEpoch(0),
      providerId: 'download',
    );
