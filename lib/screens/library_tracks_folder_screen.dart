...
class _CollectionTrackTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (kDebugMode) {
      debugPrint('_CollectionTrackTile: entry.key=${entry.key}, track.id=${track.id}, track.source=${track.source}, track.isrc=${track.isrc}');
    }
    final colorScheme = Theme.of(context).colorScheme;
    final coverFromEntry = entry.coverPath;
    final rawCoverUrl = coverFromEntry != null && File(coverFromEntry).existsSync()
        ? coverFromEntry
        : _resolveRawCoverUrl(track);
    ...
    return TrackCard(
      ...
      isSelected: isSelected,
      downloadProgress: inMemoryHistoryItem != null ? 1.0 : 0.0,
