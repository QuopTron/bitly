...
class _ExtensionDetailPageState extends ConsumerState<ExtensionDetailPage> {
  ...
  Widget _fallbackExtensionIcon(Extension ext, ColorScheme colorScheme) {
    final icon = sourceIcon(ext.id);
    if (icon != Icons.extension) {
      return Icon(icon, size: 28, color: colorScheme.onPrimaryContainer);
    }
    final fromName = sourceIcon(ext.displayName);
    if (fromName != Icons.extension) {
      return Icon(fromName, size: 28, color: colorScheme.onPrimaryContainer);
    }
                                      ...
                                      errorBuilder:
                                          (context, error, stackTrace) =>
                                            hasError
                                              ...
                                              : _fallbackExtensionIcon(extension, colorScheme),
                                    ),
                                  )
                                : hasError
                                    ? Icon(Icons.error_outline, size: 28, color: colorScheme.error)
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Share links from these sites to Bitly and this extension will handle them.',
                    style: Theme.of(context).textTheme.bodySmall?.copyWith(
                      color: colorScheme.onSurfaceVariant,
                    ),
                  ),
