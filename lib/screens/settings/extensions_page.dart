...
import 'package:bitly/providers/explore/explore_provider.dart';
import 'package:bitly/providers/settings/settings_provider.dart';
import 'package:bitly/screens/settings/download_fallback_extensions_page.dart';
import 'package:bitly/screens/settings/extension_detail_page.dart';
  ...
  Future<void> _installExtension() async {
    ...
    if (result != null && result.files.isNotEmpty) {
      ...
      final extensionPaths = selectedPaths
          .where((path) => path.toLowerCase().endsWith('.Bitly-ext'))
          .toList();
...
class _ExtensionItem extends StatelessWidget {
  ...
  Widget _fallbackIcon(ColorScheme colorScheme) {
    final icon = sourceIcon(extension.id);
    if (icon != Icons.extension) {
      return Icon(icon, size: 22, color: colorScheme.onPrimaryContainer);
    }
    final fromName = sourceIcon(extension.displayName);
    if (fromName != Icons.extension) {
      return Icon(fromName, size: 22, color: colorScheme.onPrimaryContainer);
    }
    final letter = initialLetterFor(extension.displayName);
    final color = initialColorFor(extension.displayName, colorScheme: colorScheme);
    return Text(letter,
      style: TextStyle(
        fontSize: 20,
