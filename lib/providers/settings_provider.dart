...
class SettingsNotifier extends Notifier<AppSettings> {
  ...
  Future<void> _loadSettings() async {
    ...
    try {
      ...
      _log.w('Go loadAppSettings failed, fallback: $e');
    }

    // Fallback to SharedPreferences
    if (rawSettings == null || rawSettings.isEmpty) {
      final prefs = await _prefs;
      rawSettings = prefs.getString(_settingsKey);
    }
    ...
    _premiumCheckTimer = Timer.periodic(const Duration(seconds: 10), (timer) async {
      if (state.isPremium && state.premiumUntil > 0) {
        ...
        if (!stillPremium) {
          ...
          _saveSettings();
          _log.i('Premium trial expired and revoked automatically.');
        }
      }
    });
  }
  ...
  Future<void> _saveSettings() async {
    ...
    // Save to Go backend first
    try {
      await PlatformBridge.invoke('saveAppSettings', {'value': currentJson});
    } catch (e) {
      _log.w('Go saveAppSettings failed: $e');
    }

    if (_isSavingSettings) {
      _saveQueued = true;
      _log.d('Settings save queued');
      return;
    }
  ...
  Future<void> _cleanupRetiredSpotifySettings() async {
    try {
      ...
      _log.w('Failed to cleanup retired Spotify settings: $e');
    }
  }
  ...
  Future<void> resetToFirstLaunch({bool hardReset = false}) async {
    final prefs = await _prefs;
    await prefs.remove(_settingsKey);
    await prefs.remove(_settingsCorruptBackupKey);
