...
class ExtensionNotifier extends Notifier<ExtensionState> {
  ...
  Future<void> initialize(String extensionsDir, String dataDir) async {
    ...
    try {
      ...
      // NEW: Bootstrap essential extensions from backend to ensure they are present
      ...
      // NEW: Notify the store provider to refresh so the UI reflects "Installed" state
      try {
        ref.read(storeProvider.notifier).refresh(forceRefresh: true);
      } catch (e) {
        _log.w('Could not refresh store after bootstrap: $e');
      }
  ...
  Future<void> setExtensionEnabled(String extensionId, bool enabled) async {
    try {
      ...
      if (!state.isInitialized) {
        _log.w(
          'Attempted to set extension enabled before system initialization. '
  ...
  /// Ensures default extensions are installed.
  /// NOTE: This method is now DEPRECATED and should not be called during initialization.
  /// The Go backend's BootstrapEssentialExtensions handles installation and enabling
  /// automatically during initialize(). This method is kept for manual use cases only.
  @Deprecated(
  ...
  Future<List<String>> ensureDefaultExtensionsInstalled() async {
    ...
    try {
      ...
      for (final extId in _defaultExtensionIds) {
        ...
        try {
          // Only install if the extension doesn't exist at all
          if (ext == null) {
            final tempRoot = await getTemporaryDirectory();
            final installDir = await Directory(
              '${tempRoot.path}/bootstrap_$extId',
            ...
            final success = await installExtension(downloadPath);
            if (!success) {
              _log.w('Failed to install default extension: $extId');
              continue;
            }
            installed.add(extId);
          }
          ...
          // If we need to manually enable an extension, the user should do it through UI.
        } catch (e) {
          _log.w('Failed to auto-install extension $extId: $e');
        }
      }
      ...
      _log.w('Failed to auto-install default extensions: $e');
    }
  ...
  Future<bool> ensureSpotifyWebExtensionReady({
    ...
    try {
      ...
      if (ext == null) {
        ...
        final tempRoot = await getTemporaryDirectory();
        final installDir = await Directory(
          '${tempRoot.path}/Bitly_bootstrap_spotify_web',
        ).create(recursive: true);
  ...
  static const String qobuzKennyyProviderId = 'qobuz_kennyy';
  static const String tidalMonochromeProviderId = 'tidal_monochrome';

  static const List<String> builtInDownloadProviders = [
    qobuzKennyyProviderId,
    tidalMonochromeProviderId,
  ];

  List<String> getAllDownloadProviders() {
    final extProviders = _distinctProviderIds(
      state.extensions
          .where((ext) => ext.enabled && ext.hasDownloadProvider)
          .map((ext) => ext.id),
    );
    for (final builtIn in builtInDownloadProviders) {
      if (!extProviders.contains(builtIn)) {
        extProviders.add(builtIn);
      }
    }
    return extProviders;
  }
  ...
  List<String> getAllMetadataProviders() {
    ...
    final providers = _distinctProviderIds([
      ...primarySearchMetadataExtensions,
      ...otherMetadataExtensions,
    ]);
    for (final builtIn in builtInDownloadProviders) {
      if (!providers.contains(builtIn)) {
