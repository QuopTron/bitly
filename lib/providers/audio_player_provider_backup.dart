...
class AudioPlayerNotifier extends Notifier<AudioPlayerState> {
  ...
  Future<void> prefetchVideo(String trackName, String artistName) async {
    ...
    _pendingVideoFetch = null;
          ...
          if (localVideoPath != null) {
            final mediaUri = Uri.file(localVideoPath).toString();
            _isVideoCached = true;
            _cachedVideoUrl = mediaUri;
            _videoPrefetchDone = true;
            state = state.copyWith(isVideoReady: true);
            _log.i('Local video ready: $mediaUri');
            return;
          }
           ...
           String streamUrl;
           try {
             streamUrl = await PlatformBridge.searchYouTubeVideo(
               trackName: trackName,
               artistName: artistName,
             ).timeout(const Duration(seconds: 30));
           } catch (e) {
             _log.w('YouTube search failed: $e (attempt $attempt)');
             if (attempt < maxRetries) continue;
             throw Exception('YouTube search failed after $maxRetries attempts: $e');
           }
           
           if (streamUrl.isEmpty || !streamUrl.startsWith('http')) {
             _log.w('Invalid YouTube stream URL for: $trackName - $artistName (attempt $attempt)');
             if (attempt < maxRetries) continue;
             throw Exception('Invalid YouTube stream URL after $maxRetries attempts');
           }

          _cachedVideoUrl = streamUrl;
          _isVideoCached = true;
          _videoPrefetchDone = true;
          state = state.copyWith(isVideoReady: true);
          _log.i('Video stream ready: $streamUrl');
          return;
        } catch (e) {
          _log.w('Video pre-fetch attempt $attempt failed: $e');
          if (attempt < maxRetries) {
            final delay = baseDelay * attempt;
            _log.i('Retrying in ${delay.inSeconds}s...');
            await Future.delayed(delay);
          } else {
            _log.e('Video pre-fetch failed after $maxRetries attempts: $e');
            _isVideoCached = false;
            _cachedVideoUrl = null;
            state = state.copyWith(isVideoReady: false);
          }
  ...
  Future<void> startVideo(String trackName, String artistName) async {
    ...
    if (_videoController != null && _videoReady) {
      _log.i('Video already initialized (source: $_currentVideoSource), skipping startVideo');
      return;
    }
  ...
  Future<void> _initVideoPlayer() async {
    ...
    try {
      ...
      _cachedVideoUrl = null;
      state = state.copyWith(isVideoReady: false);
      throw Exception('Video initialization failed: $e');
    }
  }
  ...
  void _startSyncMonitor() {
    ...
    _audioPositionSubscription = _player?.stream.position.listen((audioPos) {
      ...
      _videoPlayer!.stream.position.then((videoPos) {
