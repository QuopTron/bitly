...
class TrackNotifier extends Notifier<TrackState> {
  ...
  TrackNotifier(this._platformService);
  ...
  Future<void> fetchFromUrl(String url, {bool useDeezerFallback = true}) async {
    ...
    try {
      var extensionHandler = await _platformService.findURLHandler(url);
      if (extensionHandler == null) {
        ...
        if (!extensionState.isInitialized && extensionState.isLoading) {
          ...
          extensionHandler = await _platformService.findURLHandler(url);
        }
      }
      ...
      for (int attempt = 1; attempt <= 3; attempt++) {
        result = await _platformService.handleURLWithExtension(url);
        if (!_isRequestValid(requestId)) return;
  ...
  Future<void> search(
    ...
    try {
      ...
      final metadataTrackResults =
          await _platformService.searchTracksWithMetadataProviders(
            query,
            limit: 20,
  ...
  Future<void> customSearch(
    ...
    try {
      ...
      final results = await _platformService.customSearchWithExtension(
        extensionId,
        query,
        options: options,
        cancelPrevious: true,
      ...
      _log.i('Custom search returned ${results.length} items');
      _log.d('First item sample: ${results.isNotEmpty ? results[0] : 'no items'}');
      ...
      for (int i = 0; i < results.length; i++) {
        ...
        try {
          ...
          _log.d('Item $i type: $itemType, name: ${item['name']?.toString() ?? 'unknown'}');
          ...
          switch (itemType) {
            ...
            case 'track':
            default:
              tracks.add(_parseSearchTrack(item, source: extensionId));
              break;
          }
        } catch (e) {
          _log.e('Failed to parse custom search item[$i]: $e', e);
        }
      }
  ...
  Future<void> checkAvailability(int index) async {
    ...
    try {
      final availability = await _platformService.checkAvailability(
        track.id,
        track.isrc!,
      );
