...
class _MiEspacioPageState extends State<MiEspacioPage> {
  ...
  void _onThemeChanged(bool isDark) {
    ...
    sl<SettingsCache>().saveThemeMode(mode == ThemeMode.dark ? 'dark' : 'light');
  }

  void _onLanguageChanged() {
    final current = sl<ValueNotifier<Locale>>().value;
    final next = current.languageCode == 'es' ? const Locale('en') : const Locale('es');
    sl<ValueNotifier<Locale>>().value = next;
    sl<SettingsCache>().saveLanguage(next.languageCode);
  }

  FeedItem _feedItemFor(Item item) {
    final type = item.type == ItemType.album ? 'album'
        : item.type == ItemType.playlist ? 'playlist'
        : item.type == ItemType.artist ? 'artist'
        : 'track';
    return FeedItem(
      id: item.realId, type: type, name: item.title,
      artists: item.subtitle, coverUrl: item.coverUrl,
          ...
          builder: (_) => BlocProvider.value(
            ...
            child: BlocProvider.value(
              ...
              child: BlocProvider.value(
                ...
                child: AlbumDetailPage(albumId: item.realId, source: src, coverUrl: item.coverUrl)),
            ))));
      case ItemType.playlist:
        Navigator.push(context, MaterialPageRoute(
          builder: (_) => BlocProvider.value(
            ...
            child: BlocProvider.value(
              value: context.read<DownloadCubit>(),
                child: PlaylistDetailPage(collectionId: item.realId, playlistName: item.title, source: src, coverUrl: item.coverUrl)),
          )));
        ...
        Navigator.push(context, MaterialPageRoute(
          builder: (_) => BlocProvider.value(
            ...
            child: BlocProvider.value(
              ...
              child: BlocProvider.value(
                value: context.read<QueueCubit>(),
                child: ArtistDetailPage(artistId: item.realId, artistName: item.title, source: item.source)),
            ))));
      case ItemType.song:
        showSongInfoModal(context, FeedItem(
          id: item.realId, type: 'track', name: item.title,
          artists: item.subtitle, coverUrl: item.coverUrl, source: item.source,
        ));
    }
  }
  ...
  Future<void> _onCreatePlaylist() async {
    final loc = AppLocalizations.of(context);
    _playlists = await loadPlaylists(loc);
    if (mounted) setState(() {});
  }
  ...
  Future<void> _onCreatePlaylistFromLiked() async {
    ...
    final name = '${loc.setup.likedSongs} (${DateTime.now().toString().substring(0, 10)})';
    ...
    if (mounted) _showSnack('${loc.setup.miSpacePlaylist} "$name" ${loc.setup.downloaded}');
  }
  ...
  Future<void> _onCreatePlaylistFromDownloaded() async {
    ...
    try {
      ...
      final name = '${loc.setup.downloaded} (${DateTime.now().toString().substring(0, 10)})';
      final created = await playlistSvc.create(name);
      if (created == null) return;
  ...
  void _onBatchDelete(Item item) => ItemActions.batchDelete(context, _feedItemFor(item));

  Widget _retryBanner(
    BuildContext context, Responsive r, Color onBg, Color glowColor, int count,
  ) {
    final loc = AppLocalizations.of(context);
    return Padding(
      ...
      child: Container(
        ...
        child: Row(children: [
          Icon(Icons.warning_amber_rounded, size: r.footerSize + 4, color: const Color(0xFFFFA726)),
          SizedBox(width: r.spacingS),
          Expanded(
            child: Text(
              count > 1
                  ? loc.setup.downloadInterruptedMany.replaceAll('{count}', '$count')
                  : loc.setup.downloadInterruptedOne,
              style: TextStyle(fontSize: r.footerSize, color: onBg.withValues(alpha: 0.85)),
            ),
          ),
          SizedBox(width: r.spacingS),
          GestureDetector(
            ...
            child: Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.refresh, size: r.footerSize - 2, color: const Color(0xFFFFA726)),
                SizedBox(width: 4),
                Text(loc.setup.retryInterrupted,
                  style: TextStyle(
  ...
  Widget build(BuildContext context) {
    ...
    final interruptedCount =
        dlState.downloads.values.where((d) => d.state == DownloadState.interrupted).length;
    ...
    return SafeArea(
      child: Column(children: [
        ...
        MiEspacioProfile(
          ...
          onThemeChanged: _onThemeChanged, onLanguageChanged: _onLanguageChanged,
        ),
        SizedBox(height: r.spacingM),
        ...
        Expanded(
          child: GlassContainer(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              SizedBox(height: r.spacingS),
              Expanded(child: MiEspacioContent(
                loading: _loading,
                items: itemsForTab(likeState, _selectedTab, _playlists, downloadCubit: context.read<DownloadCubit>()),
