...
class DownloadOptionsSheet extends StatefulWidget {
  ...
  const DownloadOptionsSheet({
    super.key, required this.item, required this.isDark, required this.dlSettings,
...
class _DownloadOptionsSheetState extends State<DownloadOptionsSheet> {
  ...
  Future<void> _loadSizes() async {
    ...
    if (mounted) setState(() { _sizes = sizes; _hasDuration = hasDuration; });
  }
  ...
  void _start() {
    ...
    dispatchDownloads(
      cubit: cubit, commonMeta: commonMeta, settings: widget.dlSettings,
      baseId: baseId, qualityOverride: qualityOverride,
    );
    Navigator.pop(context);
  }

  Widget _buildOption(BuildContext context, Responsive r, String q, Color glowColor, Color onBg) {
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          child: Row(children: [
            _RadioButton<String>(value: q, groupValue: _selected,
              activeColor: glowColor, onChanged: (v) => setState(() => _selected = v)),
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  ...
                  if (showBestBadge) ...[
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: selected ? glowColor.withValues(alpha: 0.15) : glowColor.withValues(alpha: 0.08),
                        borderRadius: BorderRadius.circular(4),
                      ),
                      child: Text(meta.tier == 'lossless' ? 'LOSSLESS' : '',
                        style: TextStyle(
                ...
                Row(children: [
                  ...
                  if (sizeStr != null && sizeStr.isNotEmpty) ...[
                    SizedBox(width: 8),
                    Container(width: 1, height: 10, color: onBg.withValues(alpha: 0.08)),
                    SizedBox(width: 8),
                  ],
                  Text('${meta.bitrate} kbps',
                    style: TextStyle(
                      fontSize: r.footerSize - 1,
                      color: onBg.withValues(alpha: 0.35),
                    )),                    if (!_hasDuration) ...[
                    ...
                    Container(width: 1, height: 10, color: onBg.withValues(alpha: 0.08)),
                    SizedBox(width: 8),
                    Icon(Icons.hourglass_bottom, size: r.footerSize - 2, color: onBg.withValues(alpha: 0.2)),
                    SizedBox(width: 4),
                    Text(AppLocalizations.of(context).setup.unknownSize,
                      style: TextStyle(fontSize: r.footerSize - 1, fontStyle: FontStyle.italic,
  ...
  Widget _sectionHeader(BuildContext context, Responsive r, String title, Color onBg) {
    return Padding(
            ...
            Padding(
              ...
              child: Row(children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  if (widget.item.artists != null && widget.item.artists!.isNotEmpty)
                    Text(widget.item.artists!,
                      style: TextStyle(fontSize: r.footerSize, color: onBg.withValues(alpha: 0.4)),
                      maxLines: 1, overflow: TextOverflow.ellipsis),
                ])),
                SizedBox(width: r.spacingM),
                Icon(Icons.download, size: r.subtitleSize + 4, color: glowColor),
              ]),
            ),
            SizedBox(height: r.spacingM),
            // ── Lossless section ──────────────────────────────────
            _sectionHeader(context, r, 'LOSSLESS', onBg),
            ..._losslessKeys.map((q) => _buildOption(context, r, q, glowColor, onBg)),
            // ── Lossy section ────────────────────────────────────
            _sectionHeader(context, r, 'LOSSY', onBg),
            ..._lossyKeys.map((q) => _buildOption(context, r, q, glowColor, onBg)),
            SizedBox(height: r.spacingM),
            // ── Info banner ──────────────────────────────────────
            if (widget.dlSettings.videoEnabled || widget.dlSettings.lyricsEnabled)
              Padding(
                padding: EdgeInsets.symmetric(horizontal: r.spacingM),
                child: GlassContainer(
                  borderRadius: 12, borderColor: glowColor.withValues(alpha: 0.2),
                  bgColor: glowColor.withValues(alpha: 0.06),
                  padding: EdgeInsets.all(r.spacingS),
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Icon(Icons.info_outline, size: r.footerSize, color: glowColor),
                    SizedBox(width: r.spacingXS),
                    Expanded(child: Text(
                      widget.dlSettings.videoEnabled && widget.dlSettings.lyricsEnabled
              ...
              child: SizedBox(width: double.infinity, height: r.continueButtonHeight,
                child: ElevatedButton(
                  ...
                  child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    SizedBox(width: r.spacingXS),
                    Text('${loc.setup.downloaded}  ${_selected != null ? _qualities[_selected]!.label : ''}',
...
void quickDownloadTrack(FeedItem item, DownloadSettings settings) {
  ...
  dispatchDownloads(cubit: cubit, commonMeta: commonMeta, settings: settings, baseId: baseId);
}

Future<void> showDownloadOptions(BuildContext context, FeedItem item, bool isDark, {DownloadSettings? dlSettings}) async {
  final settings = dlSettings ?? await sl<SettingsCache>().getDownloadSettings();
  if (!context.mounted) return;

  // If quick download is enabled, skip the options sheet entirely
